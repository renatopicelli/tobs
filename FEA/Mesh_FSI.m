...
classdef Mesh_FSI
    ...
    methods
        ...
        %% Constructor
        function mesh = Mesh_FSI(example)
            ...
            if (nargin > 0)
                ...
                [mesh.coordinates, mesh.incidence,...
                    mesh.dirichlet_boundary, mesh.neumann_boundary] = ReadMeshP(example);
        ...
        function mesh = GenerateQuad4RectangleMesh (mesh)
            ...
            mesh.incidence(:,1) = 2*ones(nelem,1);
        ...
        % Function to select and assign structural region within domain (all the rest is fluid)
        function mesh = AssignStructuralRegion(mesh, box)
            
            % Check all elements
            for i = 1:length(mesh.centroids)
                
                % Identifier variable
                inside = true;
                
                % Check all coordinates
                for j = 1:length(box.point_1)
                    % If element centroid is outside box
                    if ((mesh.centroids(i,j) < box.point_1(j)) || (mesh.centroids(i,j) > box.point_2(j)))
                        inside = false;
                    end
                end
                
                % If element is inside the box, turn it into a structure
                if (inside)
                    mesh.incidence(i,1) = 1;
                end
            end
            
        end % end AssignStructuralRegion
        ...
        % Function to select elements from centroids information
        function list = SelectElements(mesh, box)
            
            % Initialize list
            list = zeros(length(mesh.centroids),1);
            ...
            % Final list of selected elements
            list = nonzeros(list);
            
        end % end SelectElements
