% MATLAB code to read the mesh of FEM
function [coordinates,incidence,dirichlet_boundary,neumann_boundary] = ReadMeshP(example)
    ...
    % Changing element type for this code's standard
    % Solid elements
    inci((inci(:,2) == 2),2) = 1;
    inci((inci(:,2) == 1),3) = 1;
    % Fluid elements without interface
    inci((inci(:,2) == 4),2) = 2;
    inci((inci(:,2) == 2),3) = 2;
    % Fluid elements near fluid-structure interface
    elfluido = find(inci(:,2) == 6);
    inci((inci(:,2) == 6),2) = 2;
    inci((inci(:,2) == 2),3) = 2;
    
    % Adjusting incidence matrix
    inci = [ inci(:,2) inci(:,5:8) ];
